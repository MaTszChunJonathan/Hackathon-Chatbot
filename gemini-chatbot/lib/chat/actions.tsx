...
import { ListTopTenEquip } from '@/components/flights/list-top10-equip'
...
async function submitUserMessage(content: string) {
  ...
  ;(async () => {
    try {
      /**
       * The `tools` object contains various actions that can be performed in the chat.
       * Each action has a description and a set of parameters.
       * 
       * Available actions:
       * - `showFlights`: List available flights in the UI. List 3 that match user's query.
       * - `listDestinations`: List destinations to travel cities, max 5.
       * - `showSeatPicker`: Show the UI to choose or change seat for the selected flight.
       * - `showHotels`: Show the UI to choose a hotel for the trip.
       * - `checkoutBooking`: Show the UI to purchase/checkout a flight and hotel booking.
       * - `showBoardingPass`: Show user's imaginary boarding pass.
       * - `showFlightStatus`: Get the current status of imaginary flight by flight number and date.
       */
      const result = await streamText({
        ...
        tools: {
          showTopTenEquip: {
            description:
              "Generate a list of current top 10 best selling sports equipments",
            parameters: z.object({
...
make you response easier to read with some bold and spacing. give a short bold heading for your question.
      ...
      for await (const delta of result.fullStream) {
        ...
        if (type === 'text-delta') {
          ...
          } else if (toolName === 'showTopTenEquip') {
            aiState.done({
              ...aiState.get(),
              interactions: [],
              messages: [
    ...
    .map((message, index) => ({
      ...
      display:
        message.role === 'assistant' ? (
          message.display?.name === 'showTopTenEquip' ? (
            <BotCard>
              <ListTopTenEquip summary={message.display.props.summary} />
            </BotCard>
